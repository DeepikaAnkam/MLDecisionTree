...
if __name__ == '__main__':
    ...
    for depth in range(1,6,2):
        M = np.genfromtxt('../haberman+s+survival/haberman.data', missing_values=0, skip_header=0, delimiter=',', dtype=int)
        X = M[:, :-1]
        y = M[:, -1]
        ...
        for attribute in range(len(X_train[0])):
            data = np.unique(X_train[:,attribute])
            for v in range(len(data)):
                attribute_value_pairs.append((attribute,data[v]))

        decision_tree = id3(X_train, y_train, attribute_value_pairs, max_depth=depth)

        # Pretty print it to console
        print('decision tree from id3 for depth ' + str(depth))
        pretty_print(decision_tree)

        # Visualize the tree and save it as a PNG image
        dot_str = to_graphviz(decision_tree)
        render_dot_file(dot_str, './my_learned_tree_depth_E'+str(depth))

        # Compute the test error
        y_pred1 = [predict_example(x, decision_tree) for x in X_test]
        y_pred_d[depth] = y_pred1
        ...
        # Pretty print it to console
        pretty_print_1 = tree.export_text(decision_tree)
        print('decision tree from scikit for depth ' + str(depth))
        print(pretty_print_1)

        # Visualize the tree and save it as a PNG image
        y_pred = decision_tree.predict(X_test)
        y_pred_depth[depth] = y_pred
...
for depth in range(1,6,2):
    print("Confusion Matrix for the depth " + str(depth)+"for id3 algorithm .")
    print(confusion_matrix(y_test, y_pred_d.get(depth)))
    print("\n")

    print("Confusion Matrix for the depth " + str(depth)+"for decision tree using scikit learn.")
    print(confusion_matrix(y_test, y_pred_depth.get(depth)))
    print("\n")