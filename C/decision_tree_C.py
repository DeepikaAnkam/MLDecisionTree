...
if __name__ == '__main__':
    ...
    for depth in range(1,6,2):
        ...
        decision_tree = id3(Xtrn, ytrn, attribute_value_pairs, max_depth=depth)
        ...
        render_dot_file(dot_str, './my_learned_tree_d'+str(depth))
        
        # Compute the test error
        y_pred = [predict_example(x, decision_tree) for x in Xtst]
        y_pred_d[depth] = y_pred
        tst_err = compute_error(ytst, y_pred)
        print('Test error = {0:4.2f}%.'.format(tst_err * 100))
        
from sklearn.metrics import confusion_matrix
for depth in range(1,6,2): 
    print("Confusion Matrix for depth " + str(depth))
    print(confusion_matrix(ytst, y_pred_d.get(depth)))
    print("\n")
